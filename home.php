...
$name = $_POST['username'];
$pass = $_POST['password'];
$cookie_nameuser = "user";
$cookie_namepass = "pass" ;
$cookie_valueuser = $name ;
$cookie_valuepass = $pass ;
...
if (( $name == preg_replace('/\s+/', '', $cookieguestuser) ) and ($pass == preg_replace('/\s+/', '', $cookieguestpass))) {
...
else if (($name == preg_replace('/\s+/', '', $cookieadminuser)) and ( $pass == preg_replace('/\s+/', '', $cookieadminpass))) {
	setcookie($cookie_nameuser, $cookie_valueuser, time() + (3600), "/");
	setcookie($cookie_namepass, $cookie_valuepass, time() + (3600), "/");
	...
	// nothing for you to do here except the group leader (binod kandel).
}
